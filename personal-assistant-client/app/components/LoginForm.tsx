...
export default function LoginForm() {
  ...
  const login = async (
    values: ILogin,
    onSubmitProps: FormikHelpers<ILogin>
  ) => {
    try {
      const loggedInResponse = await fetch(`${process.env.NEXT_PUBLIC_S_API}/auth/login`, {
        method: "POST",
        headers: {
      ...
      if (loggedInResponse.ok && loggedIn.access_token) {
        dispatch(
          setLogin({
